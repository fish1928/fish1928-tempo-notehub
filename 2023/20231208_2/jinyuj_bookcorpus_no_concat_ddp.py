...
class Collator_NOCAT(Collator_Base):
    ...
    def __call__(self, list_sequence_batch):
        ...
        list_tokenized_merged = [tokenized_seq[:self.size_seq_max - 2] for tokenized_seq in self.tokenizer.batch_encode_plus(list_sequence_batch, add_special_tokens=False)[
            'input_ids']]
...
def BookCorpus2000(split=0.1, num='2000'):
    filename = f'bookcorpus_{num}.json'
...
# train_source, valid_source, _ = BookCorpus(split=0.0001, used=100000)
train_source, valid_source, _ = BookCorpus2000(split=0.01, num=100000)
tokenizer = AutoTokenizer.from_pretrained("bert-base-uncased") 
collator = Collator_NOCAT(tokenizer, seq_max)
...
if __name__ == '__main__':
    ...
    #########################################################
    mp.spawn(main_run, args=(world_size,), nprocs=world_size, join=True)    
    #########################################################